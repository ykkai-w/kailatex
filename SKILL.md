---
name: kailatex
description: 把 txt / 截图整理成"简洁美观、可直接打印"的资料 PDF 的标准流程与排版规范（tectonic + ctex + Python 转换器 + PyMuPDF 预览）。
---

# kailatex — 中文资料 LaTeX → PDF 标准流程与排版规范

> 这是把"乱七八糟的 txt / 截图"整理成**一目了然、可直接打印**的中文 PDF 的固定打法。
...
1. **获取文本**
   ...
   - **截图源**：先 `Image.resize(×3, LANCZOS)` 放大（原图常 ~560px 太糊），再用 **Workflow 工作流**一图一 agent 转录（`transcribe → verify` 两遍），结构化输出；**剔除非要求内容**（Web Exercise、推荐网址、页眉页码）。
